...
static void writeExifTagAndData(int tag,
	...
	if(format == FMT_UNDEFINED){
		//add a special handle of GPSProcessingMethod 0x1B
		//for GPSProcessingMethod is unknown format but it handle it as FMT_STRING,which is diefferent
        ...
        for (i = 0; i < components && curElement != NULL; i++) {
            ...
            if (format == FMT_URATIONAL) {
                ...
                if (separator) {
					...
					//LOGD("FMT_URATIONAL is %d %d",numerator,denominator);
                    (*DataWriteIndex) += 8;
                }
